package com.example.ihjohny.trackyourself.ui.home;
...
import com.example.ihjohny.trackyourself.ui.remainingtask.RemainingTasks;
import com.example.ihjohny.trackyourself.ui.statistics.Statistics;
    ...
    private void setNextTaskCheckBox() {
        nextTaskCheckBox.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
                databaseHelper.makeCompleted(Integer.parseInt(task.getId()));
                setNextTask();
                setYourDay();
            }
        });
        ...
        if (cursor.getCount() == 0) {
            ...
            while (cursor.moveToNext()) {
                if (cursor.getString(9).equals("0")) {
                    ...
                    if (current < temp) {
                        temp = current;
                        task = new Task(cursor.getString(0), cursor.getString(1), cursor.getString(2), cursor.getString(3), cursor.getString(4), cursor.getString(5), cursor.getString(6), cursor.getString(7), cursor.getString(8), cursor.getString(9), cursor.getString(10));
                    }
                    dueCount++;
                }
            }
            if (task != null) {
                ...
                nextTaskTitleText.setText(task.getTaskName());
                nextTaskPriorityText.setText(task.getPriority());
                // nextTaskTimeText.setText(task.getHours() + " : " + task.getMinutes());
                nextTaskTimeText.setText(ExtraFunction.setRemainingTime(getContext(), task.getHours(), task.getMinutes()));
                //  nextTaskDateText.setText(ExtraFunction.setDate(getContext(),Integer.parseInt(task.getDays()),Integer.parseInt(task.getMonths()),Integer.parseInt(task.getYears())));
                if (task.getNotif().equals("1")) {
...
//        task.setColors(int [] {Color.parseColor("#000000")},getContext());
