package com.example.ihjohny.trackyourself.ui.statistics;
...
import com.example.ihjohny.trackyourself.data.db.StatisticsTaskItem;
import com.example.ihjohny.trackyourself.ui.adapter.StatisticsTaskItemAdapter;
import com.example.ihjohny.trackyourself.data.db.model.Task;
import com.example.ihjohny.trackyourself.ui.dialog.MonthYearPickerDialog;
...
public class Statistics extends AppCompatActivity implements View.OnClickListener, RadioGroup.OnCheckedChangeListener {
    ...
    private void loadTaskData(int day, int dayStart, int dayEnd, int statisticsForMonth, int statisticsForYear) {
        ...
        while (cursor.moveToNext()){
            Task dataset=new Task(cursor.getString(0),cursor.getString(1),cursor.getString(2),cursor.getString(3),cursor.getString(4),cursor.getString(5),cursor.getString(6),cursor.getString(7),cursor.getString(8),cursor.getString(9),cursor.getString(10));
            tasks.add(dataset);
        }
        ...
        for(int i = 0; i< tasks.size(); i++){
           Task task = tasks.get(i);
            ...
            if ( taskTitleMap.get(task.getTaskName())==null) {
               // StatisticsTaskItem statisticsTaskItem = new StatisticsTaskItem(task.getTaskName(), 10, i);
              //  statisticsTaskItems.add(statisticsTaskItem);
                if(task.getIsCompleted().equals("1"))
                    taskTitleMap.put(task.getTaskName(),Pair.create(1,1));
                else
                    taskTitleMap.put(task.getTaskName(),Pair.create(1,0));
            }
          else
            {
                if(task.getIsCompleted().equals("1"))
                    taskTitleMap.put(task.getTaskName(),Pair.create((taskTitleMap.get(task.getTaskName()).first+1),(taskTitleMap.get(task.getTaskName()).second+1)));
                else
                    taskTitleMap.put(task.getTaskName(),Pair.create((taskTitleMap.get(task.getTaskName()).first+1),(taskTitleMap.get(task.getTaskName()).second)));


//                taskTitleMap.put(task.getTaskName(),taskTitleMap.get(task.getTaskName())+1,);
            }
        }
